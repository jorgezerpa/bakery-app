    ...
    export function convertTimestampToMilitaryTime(timestampEnSegundos: number): string {
  // 1. Validar el timestamp
  if (typeof timestampEnSegundos !== 'number' || isNaN(timestampEnSegundos) || timestampEnSegundos < 0) {
  ...
  const date = new Date(timestampEnSegundos * 1000);
